...
const diaryApi = {
  ...
  // (property) get: (id: number) => Promise<AxiosResponse<DiaryItemResponse[], any>>
  // (method) Axios.get<DiaryItemResponse[], AxiosResponse<DiaryItemResponse[], any>, any>(url: string, config?: AxiosRequestConfig<any>): Promise<...>
  get: (id: number) =>
  ...
  // POST 요청URL HTTP/1.1  {...}
  // (property) add: (diaryItem: DiaryItemRequest) => Promise<AxiosResponse<DiaryItemResponse, any>>
  // (method) Axios.post<DiaryItemResponse, AxiosResponse<DiaryItemResponse, any>, any>(url: string, data?: any, config?: AxiosRequestConfig<any>): Promise<...>
  ...
  modify: (id: number, diaryItem: DiaryItemRequest) =>
    axios.put<DiaryItemResponse>(
      `http://ec2-3-36-96-181.ap-northeast-2.compute.amazonaws.com:8080/diary/${id}`, diaryItem)
};
